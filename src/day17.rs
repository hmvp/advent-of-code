            ...
            for (y, row) in z_planes.iter_mut().enumerate() {
                ...
                for (x, cube) in row.iter_mut().enumerate() {
                    match (
                        ...
                        (false, cubes_active) if cubes_active == 3 => {
                            *cube = true;
                        }
...
"
    ...
    fn check_get_cube_state() {
        ...
        assert_eq!(state1.get_cube_state(0, 0, 0), false);
        assert_eq!(state1.get_cube_state(0, 2, 0), true);
        assert_eq!(state1.get_cube_state(2, 1, 0), true);
    }
...
";
    ...
    #[test]
    fn check_part2() {
        assert_eq!(part2(&input_generator(INPUT)), 848);
    }
}
