...
fn deal(input: &str) -> IResult<&str, Deal> {
    let cut = map_res(preceded(tag("cut "), digit1), |n: &str| {
        n.parse().map(Deal::Cut)
    });
    let cut_minus = map_res(preceded(tag("cut -"), digit1), |n: &str| {
        n.parse().map(Deal::CutMinus)
    });
...
pub fn input_generator(input: &str) -> Vec<Deal> {
    input
        .lines()
        .flat_map(|l| deal(l.trim()).map(|(_, d)| d))
        .collect()
}
...
mod tests {
    ...
    fn check_part2() {
        ...
        assert_eq!(
            calc_shuffle(&input_generator("deal with increment 3"), 10, 0),
            0
        );
        assert_eq!(
            calc_shuffle(&input_generator("deal with increment 3"), 10, 3),
            1
        );
        assert_eq!(
            calc_shuffle(
