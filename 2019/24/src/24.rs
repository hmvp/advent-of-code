        ...
        for y in 0..HEIGHT {
            ...
            for x in 0..WIDTH {
                match (self.get_cell_state(x, y), self.neighbors_alive(x, y)) {
                    (true, 1) | (false, 1 | 2) => grid[offset + x] = true,
...
pub fn input_generator(input: &str) -> Map {
    let grid: Vec<bool> = input.lines().flat_map(|l| l.trim().chars().map(|c| c == '#')).collect();
    ...
    fn neighbors_alive(prev: &Map, this: &Map, _next: &Map, x: usize, y: usize) -> usize {
        ...
        neighbors
            ...
            + neighbors
                .iter()
                .filter(|&&(y, x)| x < 0 || x >= WIDTH as isize || y < 0 || y >= WIDTH as isize)
                .map(|&(y, x)| prev.get_cell_state(2 + (x as usize % (WIDTH - 1)), 2 + (y as usize) % (HEIGHT - 1)))
                .filter(|cell| *cell)
                .count()
            + neighbors
                .iter()
